...
questionsApi.get('/', async (c) => {
    const limitOffset = parseLimitOffset(c.req.query('limit'), c.req.query('offset'));
    ...
    const questions = await questionsClient.getQuestions(limitOffset, categorySlug);
    if (!questions.ok) {
...
questionsApi.patch('/:id', zValidator('json', QuestionCreateSchema), async (c) => {
    const id = parseId(c.req.param('id'));
    const question = c.req.valid('json');
    if (!id) {
        return c.json({ message: 'invalid id' }, 400);
